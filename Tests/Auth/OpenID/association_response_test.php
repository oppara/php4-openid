...
require_once 'Tests/Auth/OpenID/mem_store.php';
...
class Tests_Auth_OpenID_AssociationResponse extends UnitTestCase {

    function Tests_Auth_OpenID_AssociationResponse() {
        $this->UnitTestCase('Tests_Auth_OpenID_AssociationResponse');
    }
...
class TestExtractAssociationMissingFieldsOpenID2 extends Tests_Auth_OpenID_AssociationResponse {
    ...
    function TestExtractAssociationMissingFieldsOpenID2() {
        $this->UnitTestCase('TestExtractAssociationMissingFieldsOpenID2');
    }
...
class TestExtractAssociationMissingFieldsOpenID1 extends Tests_Auth_OpenID_AssociationResponse {
    ...
    function TestExtractAssociationMissingFieldsOpenID1() {
        $this->UnitTestCase('TestExtractAssociationMissingFieldsOpenID1');
    }
...
class ExtractAssociationSessionTypeMismatch extends Tests_Auth_OpenID_AssociationResponse {
    ...
    function ExtractAssociationSessionTypeMismatch() {
        $this->UnitTestCase('ExtractAssociationSessionTypeMismatch');
    }
    ...
    function _run($requested_session_type, $response_session_type, $openid1 = false) {
        ...
        $msg = mkAssocResponse($keys);
        $msg->setArg(Auth_OpenID_OPENID_NS, 'session_type', $response_session_type);
        $this->assertTrue($this->consumer->_extractAssociation($msg, $assoc_session) === null);
    }
...
class TestOpenID1AssociationResponseSessionType extends Tests_Auth_OpenID_AssociationResponse {
    ...
    function TestOpenID1AssociationResponseSessionType() {
        $this->UnitTestCase('TestOpenID1AssociationResponseSessionType');
    }
    ...
    function _run($expected_session_type, $session_type_value) {
        ...
        $error_message = sprintf('Returned sesion type parameter %s was expected ' . 'to yield session type %s, but yielded %s', $session_type_value, $expected_session_type, $actual_session_type);
        $this->assertEqual($expected_session_type, $actual_session_type, $error_message);
    }
...
class TestInvalidFields extends Tests_Auth_OpenID_AssociationResponse {
    ...
    function TestInvalidFields() {
        $this->UnitTestCase('TestInvalidFields');
    }
    ...
    function setUp() {
        ...
        $this->assoc_response = Auth_OpenID_Message::fromOpenIDArgs(array('expires_in' => '1000', 'assoc_handle' => $this->assoc_handle, 'assoc_type' => $this->assoc_type, 'session_type' => $this->session_type, 'ns' => Auth_OpenID_OPENID2_NS,));
    ...
    function test_worksWithGoodFields() {
        ...
        $assoc = $this->consumer->_extractAssociation($this->assoc_response, $this->assoc_session);
        $this->assertTrue($this->assoc_session->extract_secret_called);
        $this->assertEqual($this->assoc_session->secret, $assoc->secret);
        $this->assertEqual(1000, $assoc->lifetime);
        $this->assertEqual($this->assoc_handle, $assoc->handle);
        $this->assertEqual($this->assoc_type, $assoc->assoc_type);
    }
    ...
    function test_badAssocType() {
        ...
        $this->assoc_session->allowed_assoc_types = array();
        $this->assertTrue($this->consumer->_extractAssociation($this->assoc_response, $this->assoc_session) === null);
    }
...
class TestExtractAssociationDiffieHellman extends Tests_Auth_OpenID_AssociationResponse {
    ...
    function TestExtractAssociationDiffieHellman() {
        $this->UnitTestCase('TestExtractAssociationDiffieHellman');
    }
    ...
    function _setUpDH() {
        ...
        $this->assertEqual($this->endpoint->compatibilityMode(), $message->isOpenID1());
    ...
    function test_success() {
        ...
        $this->skipIf (Auth_OpenID_noMathSupport(),
            'no math support!'
        );
        ...
        $this->assertEqual($ret->assoc_type, 'HMAC-SHA1');
        $this->assertEqual($ret->secret, $this->secret);
        $this->assertEqual($ret->handle, 'handle');
        $this->assertEqual($ret->lifetime, 1000);
    }

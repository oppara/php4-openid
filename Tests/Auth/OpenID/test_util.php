...
class OpenIDTestMixin extends UnitTestCase {
    ...
    function failUnlessOpenIDValueEquals($msg, $key, $expected, $ns = null) {
        ...
        $error_message = sprintf($error_format, $key, $expected,$this->__getMessage( $actual));

        $this->assertEqual($expected, $actual, $error_message);
    }
    ...
    function failIfOpenIDKeyExists($msg, $key, $ns = null) {
        ...
        $actual = $this->__getMessage($msg->getArg($ns, $key));
        $error_message = sprintf('openid.%s unexpectedly present: %s', $key, $actual);

        $this->assertFalse($msg->hasKey($ns, $key), $error_message);
    }

    function __getMessage($actual) {
        if (is_object($actual)) {

...
class AXMessageTest extends UnitTestCase {

    function AXMessageTest() {
        $this->UnitTestCase('AXMessageTest');
    }
...
class AttrInfoTest extends UnitTestCase {

    function AttrInfoTest() {
        $this->UnitTestCase('AttrInfoTest');
    }
    ...
    function test_construct() {
        ...
        $this->assertEqual($type_uri, $ainfo->type_uri);
        $this->assertEqual(1, $ainfo->count);
        $this->assertFalse($ainfo->required);
        $this->assertTrue($ainfo->alias === null);
    }
}

class ToTypeURIsTest extends UnitTestCase {

    function ToTypeURIsTest() {
        $this->UnitTestCase('ToTypeURIsTest');
    }
    ...
    function test_empty() {
        foreach (array(null, '') as $empty) {
            ...
            $this->assertEqual(array(), $uris);
        }
    }

    function test_undefined() {
        $result = Auth_OpenID_AX_toTypeURIs($this->aliases, 'http://janrain.com/');
        $this->assertTrue(Auth_OpenID_AX::isError($result));
    }
    ...
    function test_one() {
        ...
        $this->assertEqual(array($uri), $uris);
    }
    ...
    function test_two() {
        ...
        $this->assertEqual(array($uri1, $uri2), $uris);
    }
}

class ParseAXValuesTest extends UnitTestCase {

    function ParseAXValuesTest() {
        $this->UnitTestCase('ParseAXValuesTest');
    }
    ...
    function failUnlessAXValues($ax_args, $expected_args) {
        ...
        $this->assertEqual($expected_args, $msg->data);
    }
    ...
    function test_countPresentButNotValue() {
        $this->failUnlessAXKeyError(array('type.foo' => 'urn:foo', 'count.foo' => '1'));
    }

    function test_invalidCountValue() {
        $msg = new Auth_OpenID_AX_FetchRequest();

        $result = $msg->parseExtensionArgs(array('type.foo' => 'urn:foo', 'count.foo' => 'bogus'));
    ...
    function test_requestUnlimitedValues() {
        ...
        $result = $msg->parseExtensionArgs($args);
    ...
    function test_countPresentAndIsZero() {
        $this->failUnlessAXValues(array('type.foo' => 'urn:foo', 'count.foo' => '0',), array('urn:foo' => array()));
    }

    function test_singletonEmpty() {
        $this->failUnlessAXValues(array('type.foo' => 'urn:foo', 'value.foo' => '',), array('urn:foo' => array()));
    }

    function test_doubleAlias() {
        // Cannot map urn:foo to alias bar. It is already mapped to alias foo
        $args = array(
            'type.foo' => 'urn:foo',
            'value.foo' => '',
            'type.bar' => 'urn:foo',
        ...
        $this->failUnlessAXKeyError($args);
    }
    ...
    function test_doubleSingleton() {
        ...
        $this->failUnlessAXValues($args, $expected_args);
    }
...
class FetchRequestTest extends UnitTestCase {

    function FetchRequestTest() {
        $this->UnitTestCase('FetchRequestTest');
    }
    ...
    function test_mode() {
        $this->assertEqual($this->msg->mode, 'fetch_request');
    }

    function test_construct() {
        $this->assertEqual(array(), $this->msg->requested_attributes);
        $this->assertEqual(null, $this->msg->update_url);

        $msg = new Auth_OpenID_AX_FetchRequest('hailstorm');
        $this->assertEqual(array(), $msg->requested_attributes);
        $this->assertEqual('hailstorm', $msg->update_url);
    }
    ...
    function test_getExtensionArgs_empty() {
        ...
        $this->assertEqual($expected_args, $this->msg->getExtensionArgs());
    }
    ...
    function test_getExtensionArgs_noAlias() {
        ...
        $this->failUnlessExtensionArgs(array('type.' . $alias => $attr->type_uri, 'if_available' => $alias));
    }

    function test_getExtensionArgs_alias_if_available() {
        $attr = Auth_OpenID_AX_AttrInfo::make('type://of.transportation', 1, false, 'transport');

        $this->msg->add($attr);
        $this->failUnlessExtensionArgs(array('type.' . $attr->alias => $attr->type_uri, 'if_available' => $attr->alias));
    }

    function test_getExtensionArgs_alias_req() {
        $attr = Auth_OpenID_AX_AttrInfo::make('type://of.transportation', 1, true, 'transport');

        $this->msg->add($attr);
        $this->failUnlessExtensionArgs(array('type.' . $attr->alias => $attr->type_uri, 'required' => $attr->alias));
    }
    ...
    function test_isIterable() {
        $this->assertEqual(array(), $this->msg->iterAttrs());
        $this->assertEqual(array(), $this->msg->iterTypes());
    }

    function test_getRequiredAttrs_empty() {
        $this->assertEqual(array(), $this->msg->getRequiredAttrs());
    }
    ...
    function test_parseExtensionArgs_extraType() {
        ...
        $this->assertTrue(Auth_OpenID_AX::isError($this->msg->parseExtensionArgs($extension_args)));
    }
    ...
    function test_parseExtensionArgs() {
        ...
        $this->assertEqual(array($this->type_a), $this->msg->iterTypes());
        ...
        $this->assertEqual($this->type_a, $attr_info->type_uri);
        $this->assertEqual($this->alias_a, $attr_info->alias);
        $this->assertEqual(array($attr_info), $this->msg->iterAttrs());
    }
    ...
    function test_extensionArgs_idempotent() {
        ...
        $this->msg->parseExtensionArgs($extension_args);
        $this->assertEqual($extension_args, $this->msg->getExtensionArgs());
    ...
    function test_extensionArgs_count1() {
        ...
        $this->msg->parseExtensionArgs($extension_args);
        $this->assertEqual($extension_args_norm, $this->msg->getExtensionArgs());
    }
    ...
    function test_openidNoRealm() {
        ...
        $openid_req_msg = Auth_OpenID_Message::fromOpenIDArgs($args);
...
class FetchResponseTest extends UnitTestCase {

    function FetchResponseTest() {
        $this->UnitTestCase('FetchResponseTestg');
    }
    ...
    function test_construct() {
        ...
        $this->assertEqual(array(), $this->msg->data);
    }
    ...
    function test_getExtensionArgs_empty() {
        ...
        $req = null;
        $this->assertEqual($expected_args, $this->msg->getExtensionArgs($req));
    }
    ...
    function test_getExtensionArgs_some_request() {
        ...
        $this->assertEqual($expected_args, $result);
    }
    ...
    function test_getSingle_success() {
        $req = new Auth_OpenID_AX_FetchRequest();
        $this->msg->addValue($this->type_a, $this->value_a);
        $this->assertEqual($this->value_a, $this->msg->getSingle($this->type_a));
    }

    function test_getSingle_none() {
        $this->assertEqual(null, $this->msg->getSingle($this->type_a));
    }
    ...
    function test_fromSuccessResponse() {
        ...
        $this->assertEqual(array($value), $values);
    }
}

class StoreRequestTest extends UnitTestCase {

    function StoreRequestTest() {
        $this->UnitTestCase('StoreRequestTest');
    }
    ...
    function test_getExtensionArgs_empty() {
        ...
        $this->assertEqual($expected_args, $args);
    }
...
class StoreResponseTest extends UnitTestCase {

    function StoreResponseTest() {
        $this->UnitTestCase('StoreResponseTest');
    }
    ...
    function test_success() {
        ...
        $this->assertEqual(array('mode' => 'store_response_success'), $msg->getExtensionArgs());
    }
    ...
    function test_fail_nomsg() {
        ...
        $this->assertFalse($msg->error_message);
        $this->assertEqual(array('mode' => 'store_response_failure'), $msg->getExtensionArgs());
    }
    ...
    function test_fail_msg() {
        ...
        $this->assertFalse($msg->succeeded());
        $this->assertEqual($reason, $msg->error_message);
        $this->assertEqual(array('mode' => 'store_response_failure', 'error' => $reason), $msg->getExtensionArgs());
    }
}

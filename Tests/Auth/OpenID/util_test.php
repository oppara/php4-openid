...
class Auth_OpenID_UtilTest extends UnitTestCase {

    function Auth_OpenID_UtilTest() {
        $this->UnitTestCase('Auth_OpenID_UtilTest');
    }
    ...
    function setUp() {
        ...
        $this->allowed_d = array();

        for ($i = 0; $i < strlen($allowed_s); $i++) {
            $c = $allowed_s[$i];
            $this->allowed_d[$c] = null;
        }
    }
    ...
    function test_base64() {
        ...
        foreach ($cases as $s) {
            $b64 = base64_encode($s);
            $this->checkEncoded($this, $b64, $this->allowed_d);
            $s_prime = base64_decode($b64);
            $this->assertEqual($s_prime, $s);
        }
        ...
        foreach (range(0, 49) as $i) {
            $n = rand(0, 2048);
            $s = implode("", array_map('chr', array_map(array($this, 'random_ordinal'), range(0, $n))));
            $b64 = base64_encode($s);
            $this->checkEncoded($this, $b64, $this->allowed_d);
            $s_prime = base64_decode($b64);
            $this->assertEqual($s_prime, $s);
    ...
    function test_urldefrag() {
        $cases = array(
            ...
            array(
                'http://foo.com/path?query',
                'http://foo.com/path?query',
            ),
            array(
                'http://foo.com/path?query=v',
                'http://foo.com/path?query=v',
            ),
            array(
                'http://foo.com/?query=v',
                'http://foo.com/?query=v',
            ),
        );
        ...
        foreach ($cases as $pair) {
            ...
            $this->assertEqual($after, $base);
            $this->assertEqual($frag, '');

            list($base, $frag) = Auth_OpenID::urldefrag($orig . "#fragment");
            $this->assertEqual($after, $base);
            $this->assertEqual('fragment', $frag);
        }
    }

    function test_normalizeUrl() {
        $this->assertEqual("http://foo.com/", Auth_OpenID::normalizeUrl("foo.com"));

        $this->assertEqual("http://foo.com/", Auth_OpenID::normalizeUrl("http://foo.com"));

        $this->assertEqual("https://foo.com/", Auth_OpenID::normalizeUrl("https://foo.com"));

        $this->assertEqual("http://foo.com/bar", Auth_OpenID::normalizeUrl("foo.com/bar"));

        $this->assertEqual("http://foo.com/bar", Auth_OpenID::normalizeUrl("http://foo.com/bar"));

        $this->assertEqual("http://foo.com/", Auth_OpenID::normalizeUrl("http://foo.com/"));

        $this->assertEqual("https://foo.com/", Auth_OpenID::normalizeUrl("https://foo.com/"));

        $this->assertEqual("https://foo.com/bar", Auth_OpenID::normalizeUrl("https://foo.com/bar"));

        $this->assertEqual("http://foo.com/bar", Auth_OpenID::normalizeUrl("HTtp://foo.com/bar"));

        $this->assertEqual("http://foo.com/bar", Auth_OpenID::normalizeUrl("HTtp://foo.com/bar#fraggle"));

        $this->assertEqual("http://foo.com/bAr/", Auth_OpenID::normalizeUrl("HTtp://fOo.com/bAr/.#fraggle"));

        if (0) {
            $this->assertEqual("http://foo.com/%E8%8D%89", Auth_OpenID::normalizeUrl("foo.com/\u8349"));

            $this->assertEqual("http://foo.com/%E8%8D%89", Auth_OpenID::normalizeUrl("http://foo.com/\u8349"));
        }
...
assert not should_raise and actual == expected, case
    ...
    function test_appendArgs() {
        ...
        foreach ($cases as $case) {
            ...
            list($url, $query) = $data;
            $this->assertEqual($expected, Auth_OpenID::appendArgs($url, $query));
        }
    }
    ...
    function test_getQuery() {
        $queries = array(
            ...
            'single&&m=x%20y&bogus' => array(
                'm' => 'x y',
            ),
            // Even with invalid encoding.  But don't do that.
            'too=many=equals&' => array(
                'too' => 'many=equals',
            ),
        );
